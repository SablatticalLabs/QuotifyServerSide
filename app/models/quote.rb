class Quote < ActiveRecord::Base
  ...
  scope :ready_to_test_for_dupes, where(" created_at > ? and (deleted != 'true' or deleted is not null) ", 2.hours.ago )
  ...
  def deleteDupes
    ...
    quotes = Quote.where( " quote_text = ? AND speaker_user_id = ? AND id != ? AND created_at > ? and (deleted != 'true' or deleted is not null) " , self.quote_text, self.speaker_user_id, self.id, self.created_at )
