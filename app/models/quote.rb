class Quote < ActiveRecord::Base
  ...
  scope :ready_to_test_for_dupes, where(" created_at > ?", 2.hours.ago).merge(Quote.not_deleted) 
  ...
  scope :ready_to_send_comment_update_email, joins(:comments).where('email_sent_flag =  ?', false).merge(Quote.not_deleted).uniq
